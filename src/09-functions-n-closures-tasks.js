...
function getPolynom(...args) {
  args.reverse();
  return (x) => (args[2] || 0) * (x ** 2) + (args[1] || 0) * x + (args[0] || 0);
}
...
function memoize(func) {
  ...
  return (...args) => {
    ...
    if (!(keyArgs in cache)) {
      cache[keyArgs] = func.apply(this, args);
    }
...
function logger(func, logFunc) {
  return (...args) => {
    const arrArgsToString = [];

    args.forEach((item) => {
      arrArgsToString.push(JSON.stringify(item));
    });

    logFunc(`${func.name}(${arrArgsToString}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${arrArgsToString}) ends`);
    return result;
  };
}
